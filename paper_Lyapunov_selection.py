"""
Evaluate the compare results of our algorithm with two base lines.

Obtain the relay decision result.
...
p_0 = 50
p_rn_tx_max = 100
C_rn_cal_max = 10000  # 10MHz = 10000kHz
C_rs_cal_max = 10000
...
# =============================================================================================
        ...
        for i in range(0, num_user):
            ...
            for j in range(0, num_RN):
                ...
                E_judge[i].append(10 * E_user_tran + 1 * Q_rn[j][t])  # E 10  Q 50
                E_judge_1[i].append(0 * E_user_tran + 1 * Q_rn[j][t])
                E_judge_2[i].append(1 * E_user_tran + 0 * Q_rn[j][t])
...
# =============================================================================================
    ...
    for y in range(0, T):
        ...
        E_local_temp_t.append(x_0)      # add there
...
plt.plot(d_time_Q, e_consu_Q, color='black', alpha=1.0,
          linestyle='--',
         label=r'$\bf{TBQM}$', markersize=5, marker=line_mark[3], linewidth=1, clip_on = False)
plt.plot(d_time_E, e_consu_E, color='black', alpha=1.0,
         linestyle='--',
         label=r'$\bf{TECM}$', markersize=5, marker=line_mark[2], linewidth=1, clip_on = False)  # marker=line_mark[12],
plt.plot(d_time_D, e_consu_D, color='black', alpha=1.0,
          linestyle='-',
         label=r'$\bf{WTBM}$', markersize=7, marker=line_mark[13], linewidth=1, clip_on = False)  # marker=line_mark[12],
...
#         color='white', alpha=1.0, hatch='///', label=r'$\bf{TBQM}$', edgecolor='black')
# plt.bar(d_time_E + width, e_consu_E, width, # edgecolor='black',
#         color='white', alpha=1.0, hatch='...', label=r'$\bf{TECM}$', edgecolor='black')
# plt.bar(d_time_D + width + width, e_consu_D, width, # edgecolor='black',
#         color='white', alpha=1.0, hatch='---', label=r'$\bf{WTBM}$', edgecolor='black')
# ax.set_xticks(d_time_E + width)      # 将坐标设置在指定位置
# ax.set_xticklabels(d_time_D)           # 将横坐标替换成
# plt.ylabel('用户平均能耗/(mW)')  # axis title
# plt.xlabel("时间/(s)")
# plt.ylim(53, 59)
